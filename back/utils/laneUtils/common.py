...
from laneUtils.dali_data import LaneDataset  # 使用我们新实现的Dataset
from laneUtils.dist_utils import get_rank, get_world_size, is_main_process, dist_print, DistSummaryWriter
...
def get_dataset_path(cfg):
    """获取数据集路径映射"""
    path_map = {
        'CULane': os.path.join(cfg.data_root, 'list/train_gt.txt'),
        'Tusimple': os.path.join(cfg.data_root, 'train_gt.txt'),
        'CurveLanes': os.path.join(cfg.data_root, 'train', 'train_gt.txt')
    }
...
def get_train_loader(cfg):
    dataset = LaneDataset(
        ...
        list_path=os.path.join(cfg.data_root, 'list/train_gt.txt'),
        mode='train',
